...
export const createTagAction = (title) => async (dispatch, getState) => {
  try {
    ...
    await tagsAPIs.createTagService(title, tokenProtection(getState));
...
export const updateTagAction = (id, title) => async (dispatch, getState) => {
  try {
    dispatch({ type: TagsConstants.UPDATE_TAG_REQUEST });
    await tagsAPIs.updateTagService(id, title, tokenProtection(getState));
