...
import { ReviewValidation } from '../Validation/NewsValidation';
import toast from 'react-hot-toast';
import { InlineError } from '../Notfications/Error';
import { Link } from 'react-router-dom';
import { reviewNewsAction } from '../../Redux/Actions/NewsActions';
...
function NewsRates({ news_ }) {
  ...
  useEffect(() => {
    if (isError) {
      toast.error(isError);
      dispatch({ type: 'CREATE_REVIEW_NEWS_RESET' });
    }
  }, [isError, dispatch]);
    ...
    <div className="my-12">
      ...
      <div className="mt-10 xl:grid flex-colo grid-cols-5 gap-12 bg-dry xs:p-10 py-10 px-2 sm:p-20 rounded-[30px]">
        <form
          ...
          <h3 className="text-2xl text-text font-semibold">Кометар "{news_?.name}"</h3>
          <p className="text-[17px] leading-7 font-medium text-border">
            Напишіть коментар до цієї новини. Він буде розміщений на цій сторінці.
          </p>
          <div className="text-[17px] w-full">
            <Select
              label="Виберіть вашу оцінку "
              options={Ratings}
              name="rating"
        ...
        <div className="col-span-3 flex w-full flex-col gap-6">
          ...
          <div className="w-full flex flex-col bg-main gap-2 rounded-lg md:p-100 p-6 h-header overflow-y-scroll">
            {news_?.reviews?.length > 0 ? (
              news_?.reviews?.map((review) => (
                <div
                  ...
                  <div className="col-span-7 flex flex-col gap-1">
                    ...
                    <p className="text-2xs leading-6 font-medium text-text">{review?.comment}</p>
                  </div>
              ...
              <Empty message={`Будьте першим, хто прокоментує "${news_?.name}"`} />
            )}
          </div>
        </div>
      </div>
